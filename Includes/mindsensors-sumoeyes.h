...
tObstacleZone MSSUMOreadZone(tSensors link) {
  ...
  //writeDebugStreamLine("SensorData is: %d", sensordata);  // writes the current value of int 'x' to the debug stream on a new line.
  ...
  else if ( sensordata > 50 && sensordata < 60 )
  {
    // obstacle is on right
    return (MSSUMO_RIGHT);
  }
  else if ( sensordata >= 65 && sensordata <= 80 )
  {
